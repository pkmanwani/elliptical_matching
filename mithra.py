...
print(f'Skin depth and wavelength{kpn1}')
window_size=5
num_points=300
...
print(f'Inside plasma parameters axisymmetric: {initial_conditions_axi}')
# Create a time grid
...
beta_x_start_axi = beta_x_values_axi[0]
alpha_x_start_axi = alpha_x_values_axi[0]
beta_y_start_axi = beta_y_values_axi[0]
alpha_y_start_axi = alpha_y_values_axi[0]
...
#print(np.flip(zs_drift+zs_drift[0]))
...
final_sigma_x_axi = pf.sigma_beam(beta_x_start_axi,emit_nx,gamma)
final_sigma_y_axi = pf.sigma_beam(beta_y_start_axi,emit_ny,gamma)
...
normalized_beam_axi_final = beam_density_axi_final/n_pe
...
print(r'Normalized beam density axi finally:' + str(normalized_beam_axi_final))
...
solution_drift = odeint(pf.ode_drift, initial_drift_axi, zs_drift)
...
solution_plasma_wrong = odeint(pf.ode_plasma, initial_drift_axi, zs_drift, args=(model, plasma_upramp_end, sigma, length, n_pe, gamma_b,ellipticity_2==1))
#Drift
...
#print(zs_all)
for z in zs_all:
    density.append(pf.plasma_density(z, model, length+4*sigma,sigma, length))
print(rf'Focal length ($L_x$,$L_y$): {-alpha_x_end_axi*beta_x_end_axi/(1+(alpha_x_end_axi**2))},{-alpha_y_end_axi*beta_y_end_axi/(1+(alpha_y_end_axi**2))}')
...
density_line, =  ax1.plot([],[], color='purple', linestyle='dotted', label=r'$Plasma$')
...
density_axi_full = np.concatenate([density_axi[::-1], density_axi])  # if symmetric

# Plot
#ax2.plot(zs_all_full, ellipticity_all_full, label='Ellipticity')
ax1.plot(zs_all, np.multiply(density,100), color='purple', label='Plasma density')
ax2.plot(zs_all, np.multiply(density,1), color='purple', label='Plasma density')
